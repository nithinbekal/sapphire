...
fn import_functions_from_file() {
    let dir = imports_dir();
    let result = run_with_dir(r#"import "./math_utils"
square(5)"#, dir);
...
fn import_class_and_instantiate() {
    let dir = imports_dir();
    let result = run_with_dir(r#"import "./point"
...
fn import_from_subdirectory() {
    let dir = imports_dir();
    let result = run_with_dir(r#"import "./sub/greeting"
greet("world")"#, dir);
...
fn import_nonexistent_file_errors() {
    ...
    let msg = format!("{}", err);
    assert!(msg.contains("not found") || msg.contains("file not found"), "unexpected: {}", msg);
}
