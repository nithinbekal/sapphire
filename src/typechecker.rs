...
use crate::ast::{CallArg, Expr, FieldDef, ParamDef, Stmt, TypeExpr};
...
impl TypeChecker {
    ...
    pub fn check(stmts: &[Stmt]) -> Vec<TypeCheckError> {
        let mut tc = Self::new();
        for stmt in stmts { tc.collect_def(stmt); }
        for stmt in stmts { tc.check_stmt(stmt); }
        tc.errors
    }
